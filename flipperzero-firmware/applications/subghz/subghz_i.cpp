/**
 * SubGHz internal helpers – C++ rewrite.
 *
 * All string_t / M*LIB string usage has been replaced with FuriString*.
 */
...
void subghz_preset_init(
    ...
    furi_string_set_str(subghz->txrx->preset->name, preset_name);
...
bool subghz_set_preset(SubGhz* subghz, const char* preset) {
    if(!strcmp(preset, "FuriHalSubGhzPresetOok270Async")) {
        furi_string_set_str(subghz->txrx->preset->name, "AM270");
    } else if(!strcmp(preset, "FuriHalSubGhzPresetOok650Async")) {
        furi_string_set_str(subghz->txrx->preset->name, "AM650");
    } else if(!strcmp(preset, "FuriHalSubGhzPreset2FSKDev238Async")) {
        furi_string_set_str(subghz->txrx->preset->name, "FM238");
    } else if(!strcmp(preset, "FuriHalSubGhzPreset2FSKDev476Async")) {
        furi_string_set_str(subghz->txrx->preset->name, "FM476");
    } else if(!strcmp(preset, "FuriHalSubGhzPresetCustom")) {
        furi_string_set_str(subghz->txrx->preset->name, "CUSTOM");
    } else {
...
void subghz_get_frequency_modulation(
    ...
    if(modulation != nullptr) {
        // Print only first two characters of preset name
        const char* name = furi_string_get_cstr(subghz->txrx->preset->name);
        furi_string_printf(modulation, "%0.2s", name);
    }
}
...
static void subghz_idle(SubGhz* subghz) {
    furi_assert(subghz);
    furi_assert(subghz->txrx->txrx_state != SubGhzTxRxStateSleep);
    furi_hal_subghz_idle();
    subghz->txrx->txrx_state = SubGhzTxRxStateIDLE;
    ...
    do {
        ...
        if(subghz->txrx->transmitter) {
            if(subghz_transmitter_deserialize(subghz->txrx->transmitter, flipper_format)) {
                const char* preset_name = furi_string_get_cstr(subghz->txrx->preset->name);
                if(strcmp(preset_name, "")) {
                    subghz_begin(
                        subghz,
                        subghz_setting_get_preset_data_by_name(subghz->setting, preset_name));
                } else {
                    FURI_LOG_E(TAG, "Unknown name preset \"%s\"", preset_name);
                    subghz_begin(
                        subghz,
                        subghz_setting_get_preset_data_by_name(subghz->setting, "AM650"));
                }
...
void subghz_tx_stop(SubGhz* subghz) {
    ...
    // Save file for dynamic protocols
...
bool subghz_key_load(SubGhz* subghz, const char* file_path, bool show_dialog) {
    ...
    do {
        ...
        if((furi_string_equal_str(temp_str, SUBGHZ_KEY_FILE_TYPE) ||
            furi_string_equal_str(temp_str, SUBGHZ_RAW_FILE_TYPE)) &&
        ...
        if(!subghz_set_preset(subghz, furi_string_get_cstr(temp_str))) break;

        if(furi_string_equal_str(temp_str, "FuriHalSubGhzPresetCustom")) {
        ...
        size_t preset_index = static_cast<size_t>(subghz_setting_get_inx_preset_by_name(
            subghz->setting, furi_string_get_cstr(subghz->txrx->preset->name)));
        subghz_preset_init(
            subghz,
            furi_string_get_cstr(subghz->txrx->preset->name),
    ...
    case SubGhzLoadKeyStateParseErr:
        if(show_dialog) dialog_message_show_storage_error(subghz->dialogs, "Cannot parse\nfile");
        return false;
...
bool subghz_save_protocol_to_file(
    ...
    do {
        ...
        if(!storage_simply_remove(storage, dev_file_name)) break;
...
bool subghz_rename_file(SubGhz* subghz) {
    ...
    if(furi_string_cmp(subghz->file_path_tmp, subghz->file_path) != 0) {
...
bool subghz_path_is_file(FuriString* path) {
    return furi_string_end_with_str(path, SUBGHZ_APP_EXTENSION);
}
...
void subghz_hopper_update(SubGhz* subghz) {
    ...
    if(subghz->txrx->txrx_state == SubGhzTxRxStateRx) subghz_rx_end(subghz);
