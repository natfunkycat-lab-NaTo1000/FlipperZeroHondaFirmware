...
void subghz_cli_command_tx(Cli* cli, string_t args, void* context) {
    ...
    if(string_size(args)) {
        int ret = sscanf(furi_string_get_cstr(args), "%lx %lu %lu", &key, &frequency, &repeat);
        if(ret != 3) {
            printf(
...
void subghz_cli_command_decode_raw(Cli* cli, string_t args, void* context) {
    UNUSED(context);
    FuriString* file_name = nullptr;
    file_name = furi_string_alloc();
    furi_string_set_str(file_name, ANY_PATH("subghz/test.sub"));
...
static void subghz_cli_command_encrypt_keeloq(Cli* cli, string_t args) {
    ...
    FuriString* source = nullptr;
    FuriString* destination = nullptr;
