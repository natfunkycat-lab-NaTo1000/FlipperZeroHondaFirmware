...
typedef struct {
    ...
    FuriString* preset_str = nullptr;
    FuriString* sample_write = nullptr;
        ...
        with_view_model(
            instance->view, (SubGhzReadRAWModel * model) {
                if(model->status == SubGhzReadRAWStatusStart) {
                    ...
                    model->ind_write = 0;
                    furi_string_set_str(model->sample_write, "0 spl.");
    ...
    case SubGhzReadRAWStatusLoadKeyTX:
        with_view_model(
            instance->view, (SubGhzReadRAWModel * model) {
                ...
                model->rssi_history_end = false;
                model->ind_write = 0;
                furi_string_set_str(model->file_name, file_name);
                furi_string_set_str(model->sample_write, "RAW");
                return true;
            });
        break;
